// src/components/roles/jefes/JefeBibliotecaPanel.tsx
...
export default function JefeBibliotecaPanel({ userName }: { userName: string; role:string}) {
  ...
  useEffect(() => {
    const fetchTramites = async () => {
      ...
      try {
        ...
        // ← SOLO TRÁMITES DE BIBLIOTECA (recursos)
        todos = todos.filter((t: any) => t.etapa === 'recursos');
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-gray-50 via-white to-purple-50">
      <DashboardHeader userName={userName} role="Jefe de Biblioteca" onMenuToggle={() => setIsSidebarOpen(!isSidebarOpen)} />
        ...
        <main className="flex-1 p-6 lg:p-10">
          <div className="max-w-7xl mx-auto space-y-8">
            <div className="text-center mb-10">
              <h1 className="text-4xl font-bold text-purple-800 mb-2">Jefe de Biblioteca</h1>
              <p className="text-xl text-gray-600">Confirmar entrega de recursos bibliográficos</p>
            </div>

            <Card className="shadow-2xl border-0">
              <CardHeader className="bg-gradient-to-r from-purple-600 to-indigo-700 text-white rounded-t-3xl">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-purple-50">
                        <TableHead className="text-purple-800 font-bold">Folio</TableHead>
                        <TableHead className="text-purple-800 font-bold">Trámite</TableHead>
                        <TableHead className="text-purple-800 font-bold">Estudiante</TableHead>
                        {activeSection === "pendientes" && <TableHead className="text-purple-800 font-bold">Acción</TableHead>}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {tramites.map((t) => (
                        <TableRow key={t.id} className="hover:bg-purple-50">
                          <TableCell className="font-mono font-bold text-purple-600">{t.folio}</TableCell>
                          ...
                          {activeSection === "pendientes" && (
                            <TableCell className="text-center">
                              <Button
                                ...
                                className="bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold"
                              >
